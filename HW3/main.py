...
def parse():
    ...
    parser.add_argument('--train_ac', action='store_true', help='whether train DQN')
...
def run(args):
    ...
    if args.train_ppo:
        env_name = args.env_name or 'LunarLander-v2'
        env = Environment(env_name, args, atari_wrapper=False)
        from agent_dir.agent_ppo import AgentPPO
        agent = AgentPPO(env, args)
        agent.train()

    if args.train_ac:
        env_name = args.env_name or 'LunarLander-v2'
        env = Environment(env_name, args, atari_wrapper=False)
        from agent_dir.agent_ac import AgentAC
