...
class BasePredictor():
    ...
    def fit_dataset(self, data, collate_fn=default_collate, callbacks=[]):
        ...
        while self.epoch < self.max_epochs:
            ...
            dataloader =torch.utils.data.dataloader.DataLoader(data, batch_size=self.batch_size,collate_fn=collate_fn)
            ...
            if self.valid is not None:
                ...
                # evaluate model
                self.model.eval()
                dataloader =torch.utils.data.dataloader.DataLoader(dataset=self.valid, batch_size=self.batch_size,collate_fn=collate_fn)
    ...
    def _run_epoch(self, dataloader, training):
        ...
        for i, batch in trange:
            ...
            if training:
                output, batch_loss = \
                    self._run_iter(batch, training) # 每iter的loss
            ...
            for metric in self.metrics:
                metric.update(output, batch) # batch 中的update
            trange.set_postfix(
                loss=loss / (i + 1),
